...
def main() -> None:
    ...
    require(subtitle_generator_h, "const Vector<bool>& wrappedIncoming", "SubtitleGenerator incoming wrap-aware ASS contract")
    ...
    require(subtitle_generator_cpp, "rowWrapped.Add(wrapped)", "SubtitleGenerator compact wrap-aware row stack")
    require(subtitle_generator_cpp, "rowWrapSteps.Add(lineHeight * 5 / 4)", "SubtitleGenerator rendered wrapped-line compensation")
    require(subtitle_generator_cpp, "rowSlots.Add(slot)", "SubtitleGenerator old scroll slot tracking")
    require(subtitle_generator_cpp, "SubtitleSlotY(data, resY, rowSlots[row])", "SubtitleGenerator slot-based scroll anchor")
    require(subtitle_generator_cpp, "wrappedOffset += rowWrapSteps[row]", "SubtitleGenerator carries wrapped row offsets upward")
    require(subtitle_generator_cpp, "lastLineWrapped", "SubtitleGenerator previous grayed wrap tracking")
    require(subtitle_generator_cpp, "incomingRow[j]", "SubtitleGenerator incoming compact row tracking")
    ...
    require(subtitle_wrap_probe_h, "int fontSize=0", "SubtitleWrapProbe custom font-size contract")
    require(subtitle_wrap_probe_h, "bool bold=true", "SubtitleWrapProbe custom bold contract")
    ...
    require(subtitle_wrap_probe_runner_h, "int probeFontSize=0", "SubtitleWrapProbeRunner custom font-size contract")
    require(subtitle_wrap_probe_runner_h, "bool probeBold=true", "SubtitleWrapProbeRunner custom bold contract")
    ...
    require(core_tests_cpp, "SubtitleGenerator keeps unwrapped rows compact", "core tests compact unwrapped spacing")
    require(core_tests_cpp, "SubtitleGenerator keeps wrapped highlighted rows in their natural slot", "core tests wrapped highlighted spacing")
    require(core_tests_cpp, "SubtitleGenerator applies wrapped highlighted clearance to the row above", "core tests wrapped highlighted clearance")
    require(core_tests_cpp, "SubtitleGenerator keeps lower incoming rows fixed below a wrapped highlighted row", "core tests lower rows fixed under wrapped highlighted row")
    require(core_tests_cpp, "SubtitleGenerator keeps wrapped grayed rows in their natural slot", "core tests wrapped grayed spacing")
    require(core_tests_cpp, "SubtitleGenerator keeps a normal-wrapped lyric compact while it is small incoming", "core tests split normal and incoming wrap spacing")
    require(core_tests_cpp, "SubtitleWrapProbe supports incoming small-size style", "core tests incoming probe style")
    ...
    require(core_tests_cpp, "[0:v]scale=-2:2160,subtitles=song.ass[v]", "core tests 4K background export command")
    require(core_tests_cpp, "[0:v]scale=-2:1080,subtitles=song.ass[v]", "core tests HD background export command")
    require(core_tests_cpp, "ExportPreviewSeconds(", "core tests export quality helper dependency")
    require(core_tests_cpp, "ExportScaleHeight(", "core tests export scale-height helper dependency")
    ...
    require(ffmpeg_export_h, "int scaleHeight=0", "ffmpeg background scale-height parameter")
    require(ffmpeg_export_h, "int canvasHeight=1080", "ffmpeg visualization canvas-height parameter")
    reject(ffmpeg_export_h, "bool uhd", "ffmpeg export non-scalable UHD flag")
    reject(ffmpeg_export_h, "bool preview", "ffmpeg export redundant preview flag")
    require(ffmpeg_export_cpp, "[0:v]scale=-2:%d,subtitles=%s[v]", "ffmpeg background parameterized scale filter")

    export_quality_h = (root / "ExportQuality.h").read_text()
    for token in ["LowRes30Sec", "LowRes60Sec", "LowResFull", "FullSource", "FullHD", "Full4K"]:
        require(export_quality_h, token, "ExportQuality enum value")
    require(export_quality_h, "enum ExportQuality", "ExportQuality enum contract")
    require(export_quality_h, "bool ExportIsLowRes(ExportQuality", "ExportQuality low-res helper")
    require(export_quality_h, "int ExportPreviewSeconds(ExportQuality", "ExportQuality preview-seconds helper")
    require(export_quality_h, "int ExportScaleHeight(ExportQuality", "ExportQuality scale-height helper")
    require(export_quality_h, "case FullSource: return 0;", "ExportQuality source-resolution entry")
    require(export_quality_h, "case Full4K:     return 2160;", "ExportQuality 4K height entry")

    visualization_cpp = (root / "Visualization.cpp").read_text()
    reject(visualization_cpp, "bool uhd", "Visualization non-scalable UHD flag")
    reject(visualization_cpp, "bool preview", "Visualization redundant preview flag")
    require(visualization_cpp, "height*1926/1080", "Visualization scalable canvas width")
    require(visualization_cpp, "w - (w % 2)", "Visualization even canvas width")
    ...
    require(ffmpeg_subtitle_probe_h, "RenderPngFrames", "ffmpeg subtitle probe PNG render declaration")
    ...
    require(ffmpeg_subtitle_probe_cpp, '"-y"', "ffmpeg subtitle probe PNG overwrite flag")
    ...
    require(project_cpp, '.Add(FullHD, "Complete, HD")', "Project HD export dropdown option")
    require(project_cpp, '.Add(Full4K, "Complete, 4K")', "Project 4K export dropdown option")
    require(project_cpp, '.Add(FullSource, "Complete, Source Resolution")', "Project source-resolution dropdown option")
    reject(project_cpp, "Complete, Full-Sized", "Project stale full-sized export dropdown label")
    require(project_cpp, 'sub.Add("HD Video..."', "Project HD export menu entry")
    require(project_cpp, 'sub.Add("4K Video..."', "Project 4K export menu entry")
    require(project_cpp, 'sub.Add("Source Resolution..."', "Project source-resolution menu entry")
    ...
    require(project_cpp, 'data.videoFilePath.StartsWith("@@")', "Project visualization detection")
    require(project_cpp, "String saveDir = Config::Get(EXPORT_DIR);\n    if (saveDir.IsEmpty()) saveDir = Config::Get(PROJECT_DIR);", "Project video export directory lookup")
    require(project_cpp, "Config::Set(EXPORT_DIR, saveDir)", "Project video export directory persistence")
    ...
    require(project_h, "void ExportVideo(ExportQuality quality", "Project video export declaration")
    require(project_h, '#include "ExportQuality.h"', "Project export quality dependency")
    require(project_h, "void RefreshExportOptions();", "Project export option refresh declaration")
    require(project_h, "bool IsVisualization() const;", "Project visualization predicate declaration")
